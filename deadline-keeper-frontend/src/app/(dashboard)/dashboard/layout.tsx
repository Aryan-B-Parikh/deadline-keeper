...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  return <div className="min-h-screen bg-background flex flex-col md:flex-row">
    <aside className="hidden md:flex flex-col w-[248px] fixed inset-y-0 z-40 bg-glass-strong border-r border-border-subtle">
      <div className="px-5 pt-6 pb-5"><Link href="/dashboard" className="flex items-center gap-3 group"><span className="flex h-9 w-9 items-center justify-center rounded-xl bg-text-primary text-white text-sm shadow-sm group-hover:rotate-[-4deg] transition-transform">DK</span><div><span className="block font-bold text-[15px] text-text-primary tracking-tight">DeadlineKeeper</span><span className="block text-[10px] font-semibold uppercase tracking-[0.14em] text-text-muted mt-0.5">Command center</span></div></Link></div>
      <div className="px-4 pb-5"><Link href="/dashboard/events/new" className="w-full flex items-center justify-between gap-2 bg-text-primary text-white min-h-[46px] px-4 rounded-2xl hover:opacity-90 hover:-translate-y-0.5 shadow-sm transition-all duration-200 font-bold text-sm"><span className="flex items-center gap-2"><Plus className="w-4 h-4" /> Add deadline</span><kbd className="hidden lg:inline-flex text-[10px] px-1.5 py-0.5 rounded-md bg-white/10 border border-white/10 font-medium">N</kbd></Link></div>
      <nav className="flex-1 px-3 space-y-1 overflow-y-auto">{navItems.map(item => { const active = pathname === item.href; return <Link key={item.href} href={item.href} className={cn('relative flex items-center gap-3 px-3 min-h-[44px] rounded-xl text-sm font-semibold transition-all duration-200', active ? 'bg-text-primary text-white shadow-sm' : 'text-text-secondary hover:bg-surface-hover hover:text-text-primary')}><item.icon className={cn('w-[18px] h-[18px]', active ? 'text-white' : 'text-text-muted')} />{item.label}{active && <span className="absolute right-3 w-1.5 h-1.5 rounded-full bg-white" />}</Link>; })}</nav>
    ...
    <div className="flex-1 md:pl-[248px] flex flex-col min-h-screen">
      <header className={cn('md:hidden sticky top-0 z-30 transition-all duration-200', scrolled ? 'bg-glass border-b border-border-subtle shadow-sm' : 'bg-transparent')}><div className="px-4 flex items-center justify-between h-16"><Link href="/dashboard" className="flex items-center gap-2"><span className="flex h-8 w-8 items-center justify-center rounded-lg bg-text-primary text-white text-[10px] font-bold">DK</span><span className="font-bold text-[15px] text-text-primary">DeadlineKeeper</span></Link><div className="w-8 h-8 bg-brand/10 text-brand rounded-full flex items-center justify-center text-sm font-bold">{user?.email?.charAt(0).toUpperCase() || '?'}</div></div></header>
      <header className={cn('hidden md:flex sticky top-0 z-30 h-16 items-center justify-end px-8 transition-all duration-200', scrolled ? 'bg-glass border-b border-border-subtle shadow-sm' : 'bg-transparent')}><div className="flex items-center gap-2 text-text-muted text-xs"><Command className="w-3.5 h-3.5" /> Your deadlines, organized.</div></header>
