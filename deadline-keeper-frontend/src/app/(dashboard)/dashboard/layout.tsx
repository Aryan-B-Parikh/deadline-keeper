...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  return <div className="min-h-screen bg-background flex flex-col md:flex-row">
    ...
    <div className="flex-1 md:pl-[248px] flex flex-col min-h-screen">
      <header className={cn('md:hidden sticky top-0 z-30 transition-all duration-200', scrolled ? 'bg-glass border-b border-border-subtle shadow-sm' : 'bg-transparent')}><div className="px-4 flex items-center justify-between h-16"><Link href="/dashboard" className="flex items-center gap-2"><span className="flex h-8 w-8 items-center justify-center rounded-lg bg-text-primary text-white text-[10px] font-bold shadow-neu-inset">DK</span><span className="font-bold text-[15px] text-text-primary">DeadlineKeeper</span></Link><div className="w-8 h-8 bg-surface text-brand shadow-neu-inset rounded-full flex items-center justify-center text-sm font-bold">{user?.email?.charAt(0).toUpperCase() || '?'}</div></div></header>
