  ...
  return (
    <div className="space-y-8">
      <section className="relative overflow-hidden rounded-[30px] border border-border-subtle bg-surface shadow-soft">
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_82%_18%,rgba(253,188,180,0.16),transparent_28%)] pointer-events-none" />
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_8%_90%,rgba(173,216,230,0.10),transparent_25%)] pointer-events-none" />
        <div className="relative p-6 sm:p-8 lg:p-10">
          <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-8">
            <div className="max-w-2xl">
              <div className="inline-flex items-center gap-2 rounded-full border border-border-subtle bg-clay-lavender/55 px-3 py-1.5 text-[11px] font-bold uppercase tracking-[0.14em] text-text-muted mb-5 shadow-neu-inset">
            ...
            <Link href="/dashboard/events/new" className="group inline-flex min-h-[48px] shrink-0 items-center justify-center gap-2 rounded-2xl bg-brand px-5 text-sm font-bold text-white shadow-neu hover:shadow-neu-hover hover:-translate-y-0.5 active:translate-y-0 active:shadow-neu-inset-strong transition-all duration-200">
          ...
          <div className="mt-9 grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-3">
            {[
              { key: 'upcoming', icon: Calendar, value: counts.upcoming, label: 'Upcoming', color: 'text-clay-blue' },
              ...
              <button key={stat.key} onClick={() => setFilter(filter === stat.key ? 'all' : stat.key as StatusFilter)} className={cn('group rounded-2xl border px-4 py-3 text-left transition-all duration-200 hover:-translate-y-0.5', filter === stat.key ? 'border-brand bg-brand text-white shadow-neu-inset' : 'border-border-subtle bg-surface-hover/65 hover:bg-surface hover:border-border-strong')} aria-pressed={filter === stat.key}>
                ...
                <span className={cn('mt-1 block text-[11px] font-semibold uppercase tracking-[0.08em]', filter === stat.key ? 'text-white/70' : 'text-text-muted')}>{stat.label}</span>
              </button>
            ))}
          </div>
        </div>
      ...
      <div className="min-h-[400px]">
        ...
        : visibleEvents.length === 0 ? <div className="flex flex-col items-center justify-center py-24 px-4 text-center bg-surface border border-border-subtle rounded-[24px] shadow-neu"><div className="w-14 h-14 bg-clay-lavender/70 text-brand rounded-2xl flex items-center justify-center mb-4 shadow-neu-inset"><Inbox className="w-7 h-7" /></div><h3 className="text-lg font-semibold text-text-primary mb-1">{filter === 'all' ? 'No deadlines here' : `No ${filter.replace('_', ' ')} deadlines`}</h3><p className="text-text-secondary text-sm max-w-sm mb-6">{filter === 'all' ? "You're completely caught up. Add a deadline when something needs your attention." : 'Try another filter to see your deadlines.'}</p>{filter === 'all' && <Link href="/dashboard/events/new" className="bg-brand text-white px-5 min-h-[44px] flex items-center rounded-xl hover:bg-brand-hover shadow-neu transition-colors text-sm font-semibold">Add deadline</Link>}</div>
